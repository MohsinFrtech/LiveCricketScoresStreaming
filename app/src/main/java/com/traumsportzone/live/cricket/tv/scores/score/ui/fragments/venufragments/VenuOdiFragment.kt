...
class VenuOdiFragment:Fragment() , AdManagerListener, NavigateData {
    private var adManagerClass: AdManager? = null
    private val venuOdiMatchesModel by lazy {
    ...
    override fun onCreateView(
        ...
        val layoutOdi = inflater.inflate(R.layout.venu_matches,container,false)
        bindingVenuMatchesFragment = DataBindingUtil.bind(layoutOdi)
        adManagerClass = AdManager(requireContext(), requireActivity(), this)
        setOdiDataForVenuMatches()
        return layoutOdi
    }

    private fun setOdiDataForVenuMatches() {
        venuOdiMatchesModel.venuMatchesList.observe(viewLifecycleOwner, Observer {
            if (!it.isNullOrEmpty()) {
                bindingVenuMatchesFragment?.noResult?.visibility(false)
                setRecyclerForT20(it)
            } else {
                bindingVenuMatchesFragment?.noResult?.text = resources.getString(R.string.noVenuODI)
                bindingVenuMatchesFragment?.noResult?.visibility(true)
            }
        })
    }

    //Setup recycler for odi venu matches....
    private fun setRecyclerForT20(liveScoresModels: List<LiveScoresModel?>) {
        val odiFormatList: MutableList<LiveScoresModel> =
            ArrayList<LiveScoresModel>()
        liveScoresModels.forEach {
            if (it?.match_format?.equals(Cons.match_format_odi, true) == true) {
                odiFormatList.add(it)
            }
        }

        val listWithAd: List<LiveScoresModel?> =
            if (Constants.nativeAdProvider != "none") {
                CodeUtils.checkNativeAd(odiFormatList)
            } else {
                odiFormatList
            }
