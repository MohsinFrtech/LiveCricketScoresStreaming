...
class VenuTestFragment:Fragment() , AdManagerListener, com.traumsportzone.live.cricket.tv.scores.streaming.utils.interfaces.NavigateData {
    private val venuTestMatchesModel by lazy {
    ...
    override fun onCreateView(
        ...
        val layoutTest = inflater.inflate(R.layout.venu_matches,container,false)
        bindingVenuMatchesFragment = DataBindingUtil.bind(layoutTest)
        adManagerClass = AdManager(requireContext(), requireActivity(), this)
        setTestDataForVenuMatches()
        return layoutTest
    }

    private fun setTestDataForVenuMatches() {
        venuTestMatchesModel.venuMatchesList.observe(viewLifecycleOwner, Observer {
            if (!it.isNullOrEmpty()) {
                bindingVenuMatchesFragment?.noResult?.visibility(false)
                setRecyclerForTest(it)
            } else {
                bindingVenuMatchesFragment?.noResult?.text = resources.getString(R.string.noVenutest)
    ...
    //Setup recycler for test venu matches....
    private fun setRecyclerForTest(liveScoresModels: List<LiveScoresModel?>) {
        val t20FormatList: MutableList<LiveScoresModel> =
            ArrayList<LiveScoresModel>()
        liveScoresModels.forEach {
            if (it?.match_format?.equals(Cons.match_format_test, true) == true) {
